...
adminRoutes.put('/config/presentaciones-participar', async (req, res, next) => {
  try {
    ...
    await pool.execute("UPDATE config_sorteo SET valor = ? WHERE clave = 'presentaiones_para_participar'", [JSON.stringify(arr)]);
...
adminRoutes.put('/config/sorteo', async (req, res, next) => {
  try {
    ...
    if (presentacionesParticipan != null) {
      await pool.execute("UPDATE config_sorteo SET valor = ? WHERE clave = 'presentaiones_para_participar'", [
